...
document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
        ...
        // Review date relative time helper
        (function initReviewDates() {
            const reviewDateElements = document.querySelectorAll('.review-date');
            if (!reviewDateElements.length) return;
            ...
            function getReviewDate(element) {
                const dateValue = element.dataset.reviewDate?.trim() || element.textContent?.trim();
            ...
            reviewDateElements.forEach(el => {
                const reviewDate = getReviewDate(el);
                if (!reviewDate) return;

                const dateLabel = formatRelativeTime(reviewDate);
                if (dateLabel) {
                    el.textContent = dateLabel;
                    el.title = new Intl.DateTimeFormat('en-US', {
